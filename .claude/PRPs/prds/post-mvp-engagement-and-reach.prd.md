...
| 3   | Comparação de Políticos (RF-POST-001)        | Rota /comparar com URL state, 2x fetchPoliticianBySlug paralelo, tabela comparativa responsiva, OG metadata, botão compartilhar    | complete    | -        | 2       | `.claude/PRPs/plans/completed/post-mvp-phase-3-comparison.plan.md` |
| 4   | Alertas de Pontuação por Email (RF-POST-002) | Tabelas alert_subscriptions + pending_subscriptions, double opt-in, AES-256-GCM para email, job pg-boss score-alert, worker Resend | complete    | -        | 3       | `.claude/PRPs/plans/completed/post-mvp-phase-4-alerts.plan.md`   |
